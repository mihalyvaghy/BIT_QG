...
MFQuantumGraph::MFQuantumGraph(int _N, int _vertices, const std::vector<QGEdge>& _edges) : N(_N), vertices(_vertices), vertex_weights(vertices) {
	edges = _edges;
	...
	bI.resize(sizeI);
	bG = Eigen::VectorXd::Zero(sizeG);

	std::vector<double> vertex_stiffness(sizeG, 0.0);
	std::vector<double> vertex_load(sizeG, 0.0);
